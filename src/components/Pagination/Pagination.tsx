...
export const Pagination: React.FC<NaviData> = props => {
  ...
  const renderNavi = () => {
    ...
    if ((props.current - props.rangePages) > 1) {
      arr.push(<li key={'page_1'} className='page-item'><span className='page-link' onClick={()=>changePage(1)}>1</span></li>);
    }
    for (let j = left; j <= props.current; j++) { 
      if (j > 0) {          
        arr.push(<li key={'page_'+j} className={currentPage(j)}><span className='page-link' onClick={()=>changePage(j)}>{j}</span></li>);
      }
    }
    // right pages
    for (let j = (props.current + 1); j <= right; j++) { 
      if (j <= total) {          
        arr.push(<li key={'page_'+j} className='page-item'><span className='page-link' onClick={()=>changePage(j)}>{j}</span></li>);
      }
    }
    if ((props.current + props.rangePages) < total) {
      arr.push(<li key={'page_'+total} className='page-item'><span className='page-link' onClick={()=>changePage(total)}>{total}</span></li>);
    }
