...
class Table extends React.Component<FetchTable> {
  ...
  renderTableHead = (data: any[]) => {
    ...
    for (let key in data[0]) {
      if (key !== 'address') {
        arrHead.push(
          <th key={key} onClick={() => this.setSort(key)}>{key} 
            {(this.state.sort.field === key && this.state.sort.ask) ? <Icon.CaretDownFill /> : <Icon.CaretUpFill />}
          </th>
        )
      } else {
        arrHead.push(
      ...
      <div className="container">  
          ...
          {(this.props.isFinished) ? 
              <div className="topButtons">  
                <div className="addButton">          
                   <button className="btn btn-primary mb-2" onClick={this.modalHandler}>Добавить</button>          
                </div>                
        ...
        <div className="row">
          <table className="table">
            <thead>
              <tr>
                {this.renderTableHead(this.props.table)}
              </tr>
            </thead>
            <tbody>
              {this.renderTable(this.sortTable(this.getFilterd(this.props.table)))}
            </tbody>
          </table>
        </div>
        ...
        />
        : null }
        {(this.state.viewRow !== null) ? <RowInfo row={this.props.table[this.state.viewRow]} /> : null }        
        {(this.state.viewModal) ? <AddForm onAdd = {this.onAdd} modalHandler = {this.modalHandler}/> : null }
      </div>
    );
  }
}
